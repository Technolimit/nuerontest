...
for i, iw in enumerate(inh_weights):
    ...
    for j, exc in enumerate(exc_values):
        ...
        results[i, j] = max_freq

X, Y = np.meshgrid(inh_weights, exc_values) 
...
for i in range(len(exc_values)):
    for j in range(len(inh_weights)):
        text = ax.text(j, i, f'{results[i, j]:.1f}',
                       ha="center", va="center", color="w", 
                       fontsize=9, fontweight='bold')

ax.set_xticks(range(len(inh_weights)))
ax.set_xticklabels([f"{x:.3f}" for x in inh_weights])
ax.set_yticks(range(len(exc_values)))
ax.set_yticklabels([f"{x:.3f}" for x in exc_values])
